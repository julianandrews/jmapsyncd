...
Task 1 (scaffold)
  ├── Task 2 (DB)
  ├── Task 3 (Maildir)
  └── Task 4 (JMAP trait + mock)
       └── Task 5 (sync engine)
            └── Task 6 (Phase 1 binary)
 Task 3 + Task 5
       └── Task 7 (Phase 2 push)
```
...
- [ ] **Task 5 — Core sync engine**
  - `sync/mailbox.rs`: mailbox list sync (create/rename/delete directories)
  - `sync/email.rs`: three-way email diff, keyword↔flag mapping
...
- [ ] **Task 6 — Phase 1 binary (daemon loop + SSE)**
...
- [ ] **Task 7 — Phase 2 (file watcher + push)**
  - `maildir/watcher.rs`: `notify` watcher, event debounce, `is_dirty` marking
